...
\begin{figure}[h!]
  ...
  \tikzstyle{voltage} = [thick, ->, >=latex]
  \tikzstyle{signal} = [->, >=stealth]

  \begin{tikzpicture}
    %\draw[help lines] (-1, -8) grid (15,1);

    \node (battery) [block, align=center] {Battery};
    \node (circuitbreaker) [block, align=center, below of=battery, yshift=-15mm] {Circuit\\Breaker};
    \node (powerboard) [block, align=center, right of=circuitbreaker, minimum height=7cm, xshift=25mm] {Power\\Distribution\\Board};
    \node (boost) [block, align=center, right of=powerboard, minimum height=2cm, xshift=25mm, yshift=25mm] {Boost\\Converter};
    \node (buck) [block, align=center, right of=powerboard, minimum height=2cm, xshift=25mm, yshift=-25mm] {Buck\\Converter};
    \node (triggersolenoid) [block, align=center, right of=boost, yshift=1cm, xshift=18mm] {Trigger\\Solenoid};
    \node (pressuresolenoid) [block, align=center, right of=boost, yshift=-1cm, xshift=18mm] {Pressure\\Solenoid};
    \node (arduino) [block, align=center, right of=buck, xshift=65mm] {Arduino};
    \node (esc) [block, align=center, below of=arduino, xshift=-25mm, yshift=-30mm] {ESC};
    \node (encoders) [block, align=center, below of=arduino, xshift=-25mm] {Encoders};
    \node (rpi) [block, align=center, right of=buck, xshift=10mm, yshift=-20mm] {Raspberry\\Pi};
    \node (pressuresensor) [block, align=center, above of=arduino, xshift=-25mm] {Pressure\\Sensor};
    \node (controlboard) [block, align=center, above of=arduino, xshift=-25mm, yshift=15mm] {Control\\Board};

    \draw [solid, thick] (battery) -- node[anchor=east] {+12V} (circuitbreaker);
    \draw [solid, thick] (circuitbreaker) -- node[anchor=north] {+12V} (powerboard);
    \draw [solid, thick] (powerboard.east) -- +(0.5,0) node[anchor=west] {+12V};
    \draw [solid, thick] (powerboard.east) -- +(0.5,0) -| +(0.5,-2.5) -| (buck.west);
    \draw [solid, thick] (powerboard.east) -- +(0.5,0) -| +(0.5,2.5) -| (boost.west);
    \draw [solid, thick] (boost.east) -- +(0.3,0) -| node[anchor=west] {+24V} +(0.3,1) -| (triggersolenoid.west);
    \draw [solid, thick] (boost.east) -- +(0.3,0) -| +(0.3,-1) -| (pressuresolenoid.west);
    \draw [solid, thick] (buck.east) -- node[anchor=south, xshift=-2.5mm] {+5V} (10.5,-5);
    \draw [solid, thick] (10.5, -5) -- (10.5, -2.5);
    \draw [solid, thick] (10.5, -5) -- (10.5, -6);
    \draw [solid, thick] (10.5, -5) -- (arduino.west);
    \draw [solid, thick] (10.5, -2.5) -- (controlboard.west);
    \draw [solid, thick] (10.5, -4) -- (pressuresensor.west);
    \draw [solid, thick] (10.5, -6) -- (encoders.west);
    \draw [solid, thick] (powerboard.south) -- node[anchor=west, yshift=1cm] {+12V} +(0,-3) -| (esc.west);
    \draw [solid, thick] (9, -5) -- (rpi.north);
    \draw [solid] (arduino.60) -- +(0,2) -| (controlboard.east);
    \draw [solid] (pressuresensor.east) -- +(1.2,0) -| (arduino.120);
    \draw [solid] (encoders.east) -- +(1.2,0) -| (arduino.240);
    \draw [solid] (esc.east) -- +(1.8,0) -| (arduino.300);
    \draw [solid] (triggersolenoid.east) -- +(1.5,0) -| (controlboard.60);
    \draw [solid] (pressuresolenoid.east) -- +(0.9,0) -| (controlboard.120);
    \draw [->] (encoders.south) -- +(0,-0.3) node[anchor=north] {Wheels};
    \draw [->] (esc.north) -- +(0,0.3) node[anchor=south] {Motors};
  \end{tikzpicture}
