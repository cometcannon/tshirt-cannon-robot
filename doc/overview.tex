...
\begin{figure}[h!]
  ...
  \tikzstyle{dcvolt} = [solid, thick, red];
  \tikzstyle{signal} = [solid, ->, >=stealth];

  \begin{tikzpicture}
    %\draw[help lines] (-6, -12) grid (12,1);
    \node (circuitbreaker) [block, align=center] {Circuit\\Breaker};
    \node (boost) [block, align=center, below of=circuitbreaker, xshift=-4cm, yshift=-2cm] {Boost\\Converter};
    \node (buck) [block, align=center, right of=boost, xshift=7cm] {Buck\\Converter};
    \node (mcu) [block, below of=buck, yshift=-1.25cm] {MCU};
    \node (triggersolenoid) [block, align=center, below of=boost, yshift=-3.5cm, xshift=-1.5cm] {Trigger\\Solenoid};
    \node (pressuresolenoid) [block, align=center, right of=triggersolenoid, xshift=2cm] {Pressure\\Solenoid};
    \node (encoders) [block, right of=pressuresolenoid, xshift=2cm] {Encoders};
    \node (pressuresensor) [block, align=center, right of=encoders, xshift=2cm] {Pressure\\Sensor};
    \node (camera) [block, align=center, right of=pressuresensor, xshift=2cm] {Camera\\Module};
    \node (esc) [block, right of=camera, xshift=2cm] {ESC};
    \node (motors) [block, below of=esc, xshift=-4.5cm, yshift=-1cm] {Motors};
